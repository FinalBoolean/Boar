            ...
            try (FileSystem fileSystem = FileSystems.newFileSystem(new File(extensionClass.getProtectionDomain().getCodeSource().getLocation().toURI()).toPath(), Collections.emptyMap())) {
                try (InputStream input = Files.newInputStream(fileSystem.getPath("config.yml"))) {
                    ...
                    if (config != null) {
                        ...
                        s = s.replace("force-server-authoritative-vehicles: true", "force-server-authoritative-vehicles: " + config.forceServerAuthoritativeVehicles());
                        s = s.replace("max-tolerance-compensated-reach: 2.91", "max-tolerance-compensated-reach: " + config.toleranceReach());
